...
public class paintJob {
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
        if(width <= 0 || height <= 0 || areaPerBucket <= 0 || extraBuckets < 0) {
            ...
            //get area of wall needed to be painted
            double areaPaint = width * height;
            //calculate how much much buckets will be needed to cover the wall
            double paintNeeded = areaPaint / areaPerBucket;
            //subtract the buckets needed to cover the wall by the extra buckets
            int paintBuy = (int) Math.ceil(paintNeeded) - extraBuckets;
            return paintBuy;
        }
    }
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket) {
        if(width <= 0 || height <= 0 || areaPerBucket <= 0) {
            ...
            double areaPaint = width * height;
            double paintNeeded = areaPaint / areaPerBucket;
            return (int) Math.ceil(paintNeeded);
        }
    }
